#!/bin/bash

export http_proxy=http://bj-rd-proxy.byted.org:3128
export https_proxy=http://bj-rd-proxy.byted.org:3128

export CALVIN_ROOT=/mnt/bn/robotics/resources/calvin
export EVALUTION_ROOT=$(pwd)

# Install dependency for calvin
# sudo apt-get -y install libegl1-mesa libegl1
# sudo apt-get -y install libgl1

# # Install dependency for dt
# sudo apt-get -y install libosmesa6-dev
# sudo apt-get -y install patchelf

# # Copy clip weights
# mkdir -p ~/.cache/clip
# cp /mnt/bn/robotics/pretrained_models/clip/ViT-B-32.pt ~/.cache/clip

# # Run
# source /mnt/bn/robotics/resources/anaconda3_arnold/bin/activate calvin_mpt

# pip3 install moviepy
# pip3 install transformers==4.38.1 -i "https://bytedpypi.byted.org/simple"
# pip3 install lightning==2.2.5

export MESA_GL_VERSION_OVERRIDE=4.1
cd $EVALUTION_ROOT
ckpt_dir=$1
config_path=$2
sudo chmod 777 -R $ckpt_dir
# source /mnt/bn/robotics/resources/anaconda3_arnold/bin/activate calvin_mpt

# python3 eval/calvin/evaluate_mp.py --config_path $config_path \
# --ckpt_path $ckpt_dir \
# --ckpt_idx 0 --raw_calvin --debug_model

# torchrun --nproc_per_node=1 --master_port=29501 eval/calvin/evaluate_ddp.py --config_path $config_path \
# --ckpt_path $ckpt_dir \
# --ckpt_idx 0 --raw_calvin --debug_model
python3 eval/calvin/evaluate_ddp.py --config_path $config_path \
--ckpt_path $ckpt_dir \
--ckpt_idx 0 --raw_calvin

# torchrun --nnodes=1 --nproc_per_node=1  --master_port=6066 eval/calvin/evaluate_ddp-v2.py --config_path $config_path \
# --ckpt_path $ckpt_dir \
# --ckpt_idx 0 --raw_calvin --debug_model