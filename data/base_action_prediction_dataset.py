...
from data.data_utils import get_prompt_builder, get_tensor_chunk, mu_law_companding, normalize_action, pad_sequences, regularize_action
...
class ActionPredictionBatchTransform:
    ...
    def convert_image(self, images: Optional[np.ndarray], image_mask: torch.Tensor, static: bool=True) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        if images is None:
    ...
    def cat_input_ids_and_action_ids(self, input_ids: List[int], action_ids: List[int], eos_token_id: Optional[int], right_pad_len: int):
        eos_offset = 1 if eos_token_id is not None else 0
        input_ids = input_ids[:len(input_ids)-eos_offset] + action_ids + input_ids[len(input_ids)-eos_offset:]
        ...
        if (right_pad_len != 0) or (not self.predict_stop_token and self.tokenizer.eos_token):
            labels[-1] = IGNORE_INDEX
        return input_ids, labels, attention_masks

    def wrap_instruction_and_action_interleave(self, task_description: str, action: torch.Tensor, action_mask: torch.Tensor):
        if self.mode == "train":
        ...
        conversation = [
            {"from": "human", "value": f"What action should the robot take to {task_description}?"\
             if self.fwd_pred_next_n == 1 else f"What {self.fwd_pred_next_n} step actions should the robot take to {task_description}?"},
            {"from": "gpt", "value": ""},
        ]
        ...
        for i in range(window_size):
            ...
            tmp_action_ids = action_ids[start_action_index*action_dim: end_action_index*action_dim]
    ...
    def wrap_instruction_and_action_segment(self, task_description, action: torch.Tensor, action_mask: torch.Tensor):
        prompt_builder = get_prompt_builder(self.model_name, eos=self.tokenizer.eos_token, bos=self.tokenizer.bos_token)
        ...
        if self.fwd_pred_next_n == 1:
            question_prompt = f"What action should the robot take to {task_description}?"
        else:
        ...
        if history_len > 0:
            ...
            if self.discrete:
                ...
                history_action_ids = [self.special_history_id for _ in range(history_len)]
        ...
        if self.discrete:
            ...
            input_ids, labels, _ = self.cat_input_ids_and_action_ids(input_ids, next_action_ids, self.tokenizer.eos_token, right_pad_len)
        else:
    ...
    def __call__(self, task_description: str, action: np.ndarray, episode_mask: np.ndarray, images: np.ndarray, gripper_images: Optional[np.ndarray]=None) -> Dict[str, Any]:
        ...
        if self.organize_type == "interleave":
            input_ids, labels, attention_mask = self.wrap_instruction_and_action_interleave(task_description, action, action_mask)
        elif self.organize_type == "segment":
            input_ids, labels, attention_mask = self.wrap_instruction_and_action_segment(task_description, action, action_mask)
        else:
            raise TypeError("The organize type must be interleave or segment")

        return dict(
            image_tensors=image_tensors, image_chunk=image_chunk, image_chunk_mask=image_chunk_mask,
            gripper_image_tensors=gripper_image_tensors, gripper_image_chunk=gripper_image_chunk,
            input_ids=input_ids, labels=labels, attention_mask=attention_mask,
            action_tensors=action, action_mask=action_mask, action_chunk=action_chunk, action_chunk_mask=action_chunk_mask
        )
...
class ActionPredictionPaddedCollator:
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        image_tensors, image_chunk, image_chunk_mask, gripper_image_tensors, gripper_image_chunk, input_ids, labels, attention_mask, action_tensors, action_mask, action_chunk, action_chunk_mask =  tuple(
            [instance[key] for instance in instances]
            for key in (
                "image_tensors", "image_chunk", "image_chunk_mask", 
                "gripper_image_tensors", "gripper_image_chunk", 
                "input_ids", "labels", "attention_mask",
                "action_tensors", "action_mask", "action_chunk", "action_chunk_mask"
            )
        )
...
class ActionPredictionDataset(BaseTaskDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        self.model_name, self.mode, self.organize_type, self.discrete, self.image_history, self.action_history, self.predict_stop_token, self.special_history_id = \
            model_name, mode, organize_type, discrete, image_history, action_history, predict_stop_token, special_history_id
            
        self.window_size, self.fwd_pred_next_n = window_size, fwd_pred_next_n
            
        self.norm_action, self.norm_min, self.norm_max, self.regular_action, self.x_mean, self.x_std, self.use_mu_law = \
    ...
    def init_batch_transform(self):
        if self.discrete:
            self.action_tokenizer = ActionTokenizer(self.tokenizer, bins=self.n_bin, min_action=self.min_action, max_action=self.max_action)
        else:
