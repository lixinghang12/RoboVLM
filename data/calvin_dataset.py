...
from data.data_utils import generate_chunck_data, get_text_function, mu_law_companding, normalize_action, regularize_action
...
from data.data_utils import get_prompt_builder, world_to_tcp_frame
...
class DiskCalvinDataset(BaseCalvinDataset):
    ...
    def _build_file_indices_lang(
        ...
        return np.array(episode_lookup), lang_lookup, right_pad_lookup, lang_ann, lang_task
    ...
    def wrap_instruction_and_action(self, lang, action, action_mask):
        ...
        for i in range(self.window_size):
            ...
            tmp_input_ids, tmp_labels = torch.tensor(tmp_input_ids), torch.tensor(tmp_labels)
    ...
    def collater(self, sample):
        ...
        if self.discrete_action:
            res = [self.wrap_instruction_and_action(s["lang"], s["actions"], s["action_mask"]) for s in sample]
            tmp_input_ids = [_[0] for _ in res]
            tmp_labels = [_[1] for _ in res]
            max_len = max([_.shape[-1] for _ in tmp_input_ids])
            instr_and_action_ids = torch.zeros((bs, self.window_size, max_len), dtype=torch.long)
            instr_and_action_labels = torch.ones((bs, self.window_size, max_len), dtype=torch.long) * -100
            instr_and_action_mask = torch.zeros((bs, self.window_size, max_len), dtype=torch.bool)

            for i in range(bs):
                instr_and_action_ids[i, :, :tmp_input_ids[i].shape[-1]] = tmp_input_ids[i]
...
if __name__ == "__main__":
    ...
    dataset = DiskCalvinDataset(
        ...
        model_name=config.get('model_name', 'vicuna'),
        n_bins=256,
        fwd_pred_next_n=config.get('fwd_pred_next_n', 1),
        key='lang',
        tcp_rel=True
    )
    samples = [dataset[55], dataset[56], dataset[0]]
    print(dataset.collater(samples))
