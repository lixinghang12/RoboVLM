...
class BaseCalvinDataset(Dataset):
    ...
    def __getitem__(self, idx: Union[int, Tuple[int, int]], fixed_seed=False) -> Dict:
        ...
        #     sequence["step_lang"].append(self.process_rt2_ag_text(action))
...
# TODO delete the BAK suffix
class DiskCalvinDataset_BAK(BaseCalvinDataset):
    """
    ...
    def collater(self, sample):
        ...
        return image_tensors, (text_tensors, attention_mask), action_tensors, gripper_tensors, state_tensors, robot_obs, raw_images


class VoltronCalvinDataset(BaseCalvinDataset):
    """
    Dataset that loads episodes as individual files from disk.
    Args:
        skip_frames: Skip this amount of windows for language dataset.
    ...
    def collater(self, sample):
        action_tensors = torch.from_numpy(np.array([np.stack(s["actions"]) for s in sample]))
        state_tensors = torch.from_numpy(np.array([np.stack(s["robot_obs"]) for s in sample]))
        image_tensors = torch.from_numpy(np.array([[np.asarray(a.resize((224,224),PIL.Image.BILINEAR), dtype=np.float32).transpose(2,0,1) for a in s["rgb_obs"]["rgb_static"]] for s in sample]))
        gripper_tensors = torch.from_numpy(np.array([[np.asarray(a.resize((224,224),PIL.Image.BILINEAR), dtype=np.float32).transpose(2,0,1) for a in s["rgb_obs"]["rgb_gripper"]] for s in sample]))
        stacked_language = [s["lang"] for s in sample]
        ...
        return image_tensors, stacked_language, action_tensors, gripper_tensors, state_tensors, robot_obs
...
def get_coco_dataset(args, image_processor, tokenizer, epoch=0):
    coco_data_dir = "/mnt/bn/llmdata/zhb/caption/coco/train2014"
    coco_ann = "/mnt/bn/llmdata/zhb/caption/coco/annotations/captions_train2014.json"
...
def get_vqa_dataset(args, image_processor, tokenizer, epoch=0):
    vqa_data_dir = "/mnt/bn/llmdata/zhb/vqa/vqav2/train2014"
    vqa_questions = "/mnt/bn/llmdata/zhb/vqa/vqav2/v2_OpenEnded_mscoco_train2014_questions.json"
    vqa_ann = "/mnt/bn/llmdata/zhb/vqa/vqav2/v2_mscoco_train2014_annotations.json"
    shared_epoch = SharedEpoch(epoch=epoch)
    preprocess_image_fn = functools.partial(
        preprocess_image, image_processor=image_processor
    )
...
def get_calvin_dataset(args, image_processor, tokenizer, epoch=0, floor=False):
    ...
    preprocess_text_fn = functools.partial(preprocess_text_calvin, tokenizer=tokenizer, decoder_type=args.decoder_type)
...
def get_sim_mode_dataset(args, image_processor, tokenizer, epoch=0, floor=False):
    dataset_path = args.calvin_dataset
    ...
    calvin_dataset = SimDatasetFull(
        image_fn=preprocess_image_fn,
        data_dir='/mnt/bn/robotics/czq/standard_webots_data/filter_data',
        text_fn=preprocess_text_fn,
        ...
        action_bin=1024,
        state_discretization=False,
        state_bin=1024,
        state_normalization=True,
        action_normalization=True,
        use_hand_observation=True,
...
def get_real_dataset(args, image_processor, tokenizer, epoch=0, floor=False):
    ...
    #     # data_dir="/mnt/bn/robotics-data-hl/real_data_cleaned/mode1_data_pick_place_001_1023/",
    #     data_dir="/mnt/bn/robotics-data-hl/real_data/mode1_data_transport_1500_001_1124_no_filter/",
    ...
    calvin_dataset = GRDataset(
        data_dir="/mnt/bn/robotics-lq2024/wht/gr2_mode3_version0_labels/gr12_seqlen10_posdiff1_labels/train",
        text_fn=preprocess_text_fn,
        seq_len=args.window_size,
    )
...
def get_real_dataset_concat(args, image_processor, tokenizer, epoch=0, floor=False):
    
    data_dir_map = {
        "Mode1": "/mnt/bn/robotics-data-hl/real_data/gr2_labels_1110/Mode1/train",
        "Mode3": "/mnt/bn/robotics-data-hl/real_data/gr2_labels_1110/Mode3/train",
        "CALVIN": "/mnt/bn/robotics-data-hl/real_data/gr2_labels_1110/CALVIN/task_ABCD_D/train",
        "Bridge": "/mnt/bn/robotics-data-hl/real_data/gr2_labels_1110/Bridge/train",
        "RT-1": "/mnt/bn/robotics-data-hl/real_data/gr2_labels_1110/RT-1/train"
    }
    ...
    def make_configs():
        configs = {
            ...
            "datasets": [
                {
                    "type": "GRDataset",
                    "data_dir": data_dir_map[data_dir],
                    "c_act_scaler": [50.0, 50.0, 50.0, 33.0, 33.0, 33.0]
                }
                for data_dir in args.real_data_dirs
            ],
            "sampler": {
                "type": "DistributedWeightedSampler",
                "weights": args.sample_weights,
    ...
    num_samples = args.sample_per_epoch
    global_batch_size = args.batch_size_calvin * args.world_size
    num_batches = round_fn(num_samples / global_batch_size)
    num_workers = max(1, args.workers)
    num_worker_batches = round_fn(num_batches / num_workers)  # per dataloader worker
    ...
    dataloader = DataLoader(
        ...
        sampler=sampler,
        persistent_workers=True,
        collate_fn=concat_dataset.collator,
        drop_last=True
    )
    # dataloader = DataLoader(calvin_dataset, batch_size=args.batch_size_calvin)
    ...
    return DataInfo(dataloader=dataloader, shared_epoch=shared_epoch, sampler=sampler, dataset=concat_dataset)
...
def get_voltron_dataset(args, image_processor, tokenizer, epoch=0, floor=False):
    dataset_path = args.calvin_dataset
    ...
    calvin_dataset = VoltronCalvinDataset(
        datasets_dir=Path(dataset_path) / "training",
        window_size=args.window_size,
        rgb_pad=args.rgb_pad,
        gripper_pad=args.gripper_pad,
...
def get_dataset_fn(dataset_type):
    ...
    elif dataset_type == "real_concat":
        return get_real_dataset_concat
    elif dataset_type == "coco":
        return get_coco_dataset
    elif dataset_type == "vqa":
        return get_vqa_dataset
    elif dataset_type == "voltron":
        return get_voltron_dataset
    elif dataset_type == "sim_mode":
        return get_sim_mode_dataset
    else:
...
if __name__ == '__main__':
    data_path = '/mnt/bn/apretrain/RT-X-data-process/data/anns/ego4d/gr2-debug/train/json/'
    data_path = "/mnt/bn/robotics-data-hl/real_data/new_data/CALVIN/task_ABCD_D/train"
    data_path = "/mnt/bn/robotics-data-hl/real_data/gr2_labels_1110/CALVIN/task_ABCD_D/val"
    data_path = "/mnt/bn/robotics-data-hl/real_data/gr2_labels_1110/Mode1/val"
    data_path = '/mnt/bn/robotics-lq2024/wht/gr2_mode3_version0_labels/gr12_seqlen10_posdiff1_labels/train'
    data_path = "/mnt/bn/robotics/manipulation_data/calvin_data/task_ABCD_D/training"
    ...
    dataset = DiskCalvinDataset(
        datasets_dir=Path(data_path),
        text_fn=preprocess_text_fn,
        window_size=5,
        image_fn=functools.partial(preprocess_image, image_processor=clip_preprocess)
    )
    dataloader = DataLoader(dataset, batch_size=2, shuffle=False, num_workers=2, collate_fn=dataset.collater)
