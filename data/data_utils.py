...
def get_text_function(tokenizer, tokenizer_type, max_length):
    ...
    if tokenizer_type == 'flamingo':
        ...
        def preprocess_text_llava(sample, tokenizer):
            ...
            tokenizer.padding_side = "right"
            sample = [
                (f"{tokenizer.bos_token}{system_prompt}{rgb_prompt}{DEFAULT_IMAGE_TOKEN}{gripper_prompt}{DEFAULT_IMAGE_TOKEN}{s.strip()}{tokenizer.eos_token}") for s in sample
            ]
            text = tokenizer(
                sample,
                max_length=max_length,
        ...
        raise NotImplementedError(f"Invalid tokenizer type {tokenizer_type}")
...
def build_logger(logger_name, logger_filename):
    global handler
    from data.vid_llava_constants import LOGDIR
...
def get_llava_video_processor(tokenizer, video_decode_backend='decord', num_frames=8):
    from data.llava_vid_processor import LanguageBindVideoProcessor
...
def get_prompt_builder(model_name, eos=None, bos=None):
    ...
    import data.prompting
    if 'vicuna' in model_name.lower():
        return data.prompting.VicunaV15ChatPromptBuilder(model_family, eos=eos, bos=bos)
    elif 'mistral' in model_name.lower():
        return data.prompting.MistralInstructPromptBuilder(model_family, eos=eos, bos=bos)
...
def get_chunked_episode(window_sample: Literal["sliding", "range"], left_pad: bool, window_size: int, fwd_pred_next_n: int, episode_idx_range: np.ndarray):
...
def pad_sequences(sequences: List[torch.Tensor], padding_value):
    ...
    padded_tensors = [
        F.pad(tensor, (0, max_len - tensor.shape[-1]), mode='constant', value=padding_value) 
...
def world_to_tcp_frame(action, robot_obs):
    with autocast(dtype=torch.float32):
        ...
        # upscaling again
        orn_tcp_rel *= 100
        action_tcp = torch.cat([pos_tcp_rel.reshape(b, s, -1), orn_tcp_rel.reshape(b, s, -1), action[..., -1:]], dim=-1)
        if flag:
...
def tcp_to_world_frame(action, robot_obs):
    with autocast(dtype=torch.float32):
        ...
        # upscaling again
        orn_w_rel *= 100
        action_w = torch.cat([pos_w_rel.reshape(b, s, -1), orn_w_rel.reshape(b, s, -1), action[..., -1:]], dim=-1)
        if flag:
