...
def ImageTextDataset(
    ...
    data_list = list_dir_with_cache(data_dir, cache_dir='/mnt/bn/apretrain/cache')
    data_list = [f for f in data_list if f.endswith(".parquet")]
    random.shuffle(data_list)
    split = 'train' if is_training else 'test'
    ds = datasets.load_dataset(
        "parquet", data_files=data_list, split=split, streaming=True).shuffle(
...
if __name__ == "__main__":
    dataset = ImageTextDataset(
        "/mnt/bn/llmdatahl/zhb/pretraining/coyo700m/loading_successful/037",
        tokenizer={
            "type": "GPT2Tokenizer",
