...
from data.calvin_dataset_test import BaseCalvinDataset
...
class PatchMask(nn.Module):
    ...
    def forward(self, x):
        ...
        # TODO too slow, but step by patch size can be faster.
        mask_coords = []
...
class RealDatasetHDF5(Dataset):
    def __init__(self,
        ...
        if self.use_data_augmentation:
            ...
            if patch_mask:
                trans.append(PatchMask(mask_ratio=0.2))
            self.static_rgb_preprocess_train = T.Compose(trans)
            trans = [
            ...
            if patch_mask:
                trans.append(PatchMask(mask_ratio=0.2))
            self.hand_rgb_preprocess_train = T.Compose(trans)
        else:
    ...
    def _initialize(self):
        ...
        #     traj_state = np.zeros((n_frames, 6)).astype(np.float32)
        ...
        #     self.seq_tuple.append([traj_idx])
    ...
    def __getitem__(self, index):
        ...
        # if self.action_mode == 'ee_rel_pose':
        #     xyz_action = (self.robot_states[traj_idx][1:, :3] - self.robot_states[traj_idx][:-1, :3])
        #     rpy_action = (self.robot_states[traj_idx][1:, 3:6] - self.robot_states[traj_idx][:-1, 3:6])
        # elif self.action_mode == 'ee_rel_pose_local':
        #     curr_xyz = self.robot_states[traj_idx][:-1, :3]
        #     curr_rpy = self.robot_states[traj_idx][:-1, 3:6]
        #     curr_rotm = euler2rotm(curr_rpy)
        #     next_xyz = self.robot_states[traj_idx][1:, :3]
        #     next_rpy = self.robot_states[traj_idx][1:, 3:6]
        ...
        xyz_action = (self.robot_states[traj_idx][1:, :3] - self.robot_states[traj_idx][:-1, :3])
        rpy_action = (self.robot_states[traj_idx][1:, 3:6] - self.robot_states[traj_idx][:-1, 3:6])
        return xyz_action, rpy_action
    
    def count(self):
        # xyz_action_list = []
        # rpy_action_list = []
        # for i in tqdm(range(len(self.seq_tuple))):
        #     xyz_action, rpy_action = self.__getitem__(i)
        #     xyz_action_list.append(xyz_action)
        #     rpy_action_list.append(rpy_action)

        # xyz_actions = np.concatenate(xyz_action_list, axis=0)
        # rpy_actions = np.concatenate(rpy_action_list, axis=0)
        # np.save(os.path.join(self.dataset_dir, "xyz_actions.npy"), xyz_actions)
        # np.save(os.path.join(self.dataset_dir, "rpy_actions.npy"), rpy_actions)
        xyz_actions = np.load(os.path.join(self.dataset_dir, "xyz_actions.npy"))
        rpy_actions = np.load(os.path.join(self.dataset_dir, "rpy_actions.npy"))
        BaseCalvinDataset.count_statics(xyz_actions, rpy_actions)
...
if __name__ == "__main__":
    ...
    # data_dir = "/mnt/bn/robotics-lq2024/wht/gr2_mode3_version0_labels/gr12_seqlen10_posdiff1_labels/"
