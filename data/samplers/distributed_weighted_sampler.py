...
class DistributedWeightedSampler(Sampler[int]):
    ...
    def __init__(self, dataset: ConcatDataset, weights: Optional[Sequence[float]] = None,
                 sample_per_epoch: Optional[int] = None, num_replicas: Optional[int] = None,
                 rank: Optional[int] = None, shuffle: bool = True, seed: int = 0,
                 drop_last: bool = False, is_training=True) -> None:
    ...
    def _refine_num_samples_per_dataset(self, original_samples_per_dataset: Sequence[int]):
        """
        ...
        if self.is_training and self.sample_per_epoch is not None:
            ...
            original_samples_per_dataset = [math.ceil(n * self.sample_per_epoch / total_num_samples)
        ...
        if self.drop_last and total_num_samples % self.num_replicas != 0:  # type: ignore[arg-type]
            ...
            num_samples = [
                n - n_drop - 1 if i < total_drop_num_samples % self.num_datasets
                else n - n_drop for i, n in enumerate(original_samples_per_dataset)
            ]
            assert sum(num_samples) == total_num_samples - total_drop_num_samples

        else:
            total_add_num_samples = self.num_replicas - total_num_samples % self.num_replicas
            ...
            num_samples = [
                n + n_add + 1 if i < total_add_num_samples % self.num_datasets
                else n + n_add for i, n in enumerate(original_samples_per_dataset)
            ]
