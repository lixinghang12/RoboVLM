...
import torch
import transformers
from data.data_utils import get_llava_image_processor, get_llava_video_processor

from data.vid_llava_constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, \
    DEFAULT_IM_END_TOKEN, DEFAULT_VIDEO_TOKEN, DEFAULT_VID_START_TOKEN, DEFAULT_VID_END_TOKEN, MAX_IMAGE_LENGTH, \
    MAX_VIDEO_LENGTH
from torch.utils.data import Dataset

from data import conversation as conversation_lib
from data.vid_llava_mm_utils import tokenizer_image_token

from PIL import Image
from data.data_utils import order_pick_k
from utils.model_utils import build_tokenizer
from data.data_utils import get_text_function
...
class VideoLLaVADataset(LazySupervisedDataset):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.collater = DataCollatorForSupervisedDataset(tokenizer=tokenizer)
        super(VideoLLaVADataset, self).__init__(data_path=data_paths, tokenizer=tokenizer, data_args=data_args)
...
if __name__ == "__main__":
    ...
    # tokenizer = AutoTokenizer.from_pretrained('/mnt/bn/robotics-data-lxh-lq/LLaVA/llava-v1.6-vicuna-7b', local_files_only=False, trust_remote_code=True)
    # tokenizer.pad_token = tokenizer.unk_token
    # conversation_lib.default_conversation = conversation_lib.conv_templates["v1"]
    # model_name = "vicuna"
    
    tokenizer = AutoTokenizer.from_pretrained('/mnt/bn/robotics-data-lxh-lq/lxh/Qwen-VL', local_files_only=False, trust_remote_code=True, padding_side="right",)
    ...
    data_args = DataArguments()
    data_args.data_path = [
                            '/mnt/bn/robotics-data-lxh-lq/vl_data/Video-LLaVA/train_json/llava_image_.json', 
                            # '/mnt/bn/robotics-data-lxh-lq/vl_data/Video-LLaVA/train_json/llava_image_tune_.json', 
                        #    '/mnt/bn/robotics-data-lxh-lq/vl_data/Video-LLaVA/train_json/videochatgpt_tune_.json'
                           ]
    data_args.image_folder = '/mnt/bn/robotics-data-lxh-lq/vl_data/Video-LLaVA'
    data_args.video_folder = '/mnt/bn/robotics-data-lxh-lq/vl_data/Video-LLaVA'
    data_args.image_processor = image_processor
    data_args.video_processor = video_processor
    # data_loader = make_supervised_data_module(tokenizer=tokenizer, data_args=data_args)
    dataset = VideoLLaVADataset(data_args.data_path, tokenizer, image_folder = '/mnt/bn/robotics-data-lxh-lq/vl_data/Video-LLaVA', video_folder = '/mnt/bn/robotics-data-lxh-lq/vl_data/Video-LLaVA', model_name=model_name)
    data_loader = DataLoader(dataset, batch_size=2, collate_fn=dataset.collater)
