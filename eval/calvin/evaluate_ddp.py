...
from eval.calvin.model_wrapper import CustomModel, CustomModelLLaVA, CustomModelFlamingoVideo
...
def evaluate_ddp(rank, world_size, model_fn, model_args, dataset_path, sr_path, result_path, ckpt_dir, debug):
    setup(rank, world_size)
    torch.cuda.set_device(rank)
    env = make_env(dataset_path)
    model_args['device'] = torch.device(f"cuda:{rank}")
    model = model_fn(**model_args)
    if rank == 0:
        print('-'*9)
        print(sr_path)
    results = evaluate_policy(rank, world_size, model, env, sr_path, ckpt_dir, debug)
    results = torch.tensor(results).to(rank)
    if rank == 0:
        gather_list = [torch.zeros_like(results) for _ in range(world_size)]
        dist.gather(results, gather_list=gather_list, dst=0)
        gather_list = [each.to(rank) for each in gather_list]
        results = torch.concat(gather_list, dim=0).tolist()
    ...
    if not args.no_cache:
        ...
        with open(cache_file, 'w') as f:
            _info = {
                ...
                "config_raw_path": configs.get('config_raw_path', None)
            }
            json.dump(_info, f, indent=2)
    return model_fn, model_args, args.dataset_path, sr_path, result_path, ckpt_dir, args.debug, 

def clean_after_ddp(args):
    if args.no_cache:
