...
from model.text_encoder.clip_text_encoder import ClipTextFeatureEncoder
...
class RoboLLaVA(nn.Module):
    ...
    # def state_dict(self):
    #     trainable_state_dict = {k: v for k, v in self.named_parameters() if v.requires_grad}
    #     return trainable_state_dict

    def _init_llava(self):
        tokenizer, llava_model = build_llava(self.llava_path, self.llava_config['precision'])
    ...
    def encode_images(self, images, image_sizes=None):
        ...
        if type(images) is list or images.ndim == 5:
            ...
            # cur = time.time()
            # print("encode image consumes {}".format(cur-st))
            # st = cur
        ...
        # cur = time.time()
        # print("reshape image feature consumes {}".format(cur-st))
        # st = cur
        ...
        if self.use_vision_resampler:
            ...
            # cur = time.time()
            # print("resampling image consumes {}".format(cur-st))
            # st = cur
    ...
    def prepare_inputs_labels_for_action_prediction(
        ...
        **kwargs
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            cur_new_input_embeds = [self.llava_model.get_model().embed_tokens(cur_input_ids)]
    ...
    def merge_multi_modal_input(
        ...
        if is_raw_feature:
            ...
            rgb_feats = vision_x
        # print(input_embeds.shape, rgb_feats.shape)
        ...
        if attention_mask is not None:
            ...
            rgb_mask = torch.full(
                (rgb_feats.shape[0], rgb_feats.shape[1]), val, dtype=attention_mask.dtype, device=attention_mask.device
            )
            multimodal_attention_mask = torch.cat(
                [attention_mask[:, :insert_idx], rgb_mask, attention_mask[:, insert_idx:]],
                dim=1
            )

        mutlimodal_labels = None
        if labels is not None:
            rgb_labels = torch.full(
                (rgb_feats.shape[0], rgb_feats.shape[1]), -100, dtype=labels.dtype, device=labels.device
            )
            mutlimodal_labels = torch.cat(
                [labels[:, :insert_idx], rgb_labels, labels[:, insert_idx:]],
                dim=1
            )
    ...
    def _init_heads(self):
        ...
        if self.fwd_head_configs is not None:
            ...
            _kwargs.update(dict(image_size=self.vision_tower.config.image_size,
    ...
    def forward_vl_task(
        ...
        **kwargs
        ...
        output = self.llava_model(
            ...
            resampler=self.vision_resampler
        )
    ...
    def forward_discrete(
        ...
        input_embeds = self.llava_model.get_model().embed_tokens(instr_and_action_ids)
    ...
    def forward_continuous_v2(
        ...
        input_embeds = self.llava_model.get_model().embed_tokens(lang_x)
        # get <bos> & <eos> offset
        ...
        # cur = time.time()
        # print("forward token sequence consumes {} sec".format(cur-st))
        # st = cur

        output_hs = output.hidden_states[-1].clone()
        # output_attn = output.attentions
    ...
    #             if token_src == 'text':
    #                 action_hs = torch.masked_select(action_hs, new_lang_mask.cuda().bool().unsqueeze(-1)).reshape(bs, seq_len, -1, action_hs.shape[-1])
    #             elif token_src == 'all':
    #                 action_hs = action_hs.reshape(bs, seq_len, *action_hs.shape[1:])
    #             else:
    #                 raise ValueError(f"Unsupported token source {token_src}")
    ...
    def pred_action_discrete(
        ...
        for i in range(action_dim*self.fwd_pred_next_n):
            if kv_cache is None:
                output_hs = self.llava_model(inputs_embeds=multimodal_embeds, past_key_values=kv_cache, use_cache=True)
            else:
                output_hs = self.llava_model(inputs_embeds=multimodal_embeds[:, -1:], past_key_values=kv_cache, use_cache=True)
            kv_cache = output_hs.past_key_values
            cur_id = output_hs.logits[:, -1].argmax(dim=-1)
            generated_ids.append(cur_id)
            cur_embed = self.llava_model.get_model().embed_tokens(cur_id)
...
if __name__ == "__main__":
    ...
    configs = load_config('configs/llava_new/llava_ph/finetune_vicuna-7b_cont-gpt-post_full_ft_text_resam-64_wd=1e-6_hist=8_act=10_use-hand_aug-shift.json')
