...
class BasePolicyHead(nn.Module):
    ...
    def loss(self, pred_action, labels, attention_mask=None):
        ...
        if attention_mask is None:
            ...
            gripper_loss = torch.nn.functional.binary_cross_entropy_with_logits(pred_action[..., -1], labels[1], reduction='none')
...
class DiscreteDecoder(BasePolicyHead):
    ...
    def __init__(self, 
        ...
        super().__init__(hidden_size, action_dim, action_space, down_sample, latent, **kwargs)
        ...
        self.action_tokenizer = ActionTokenizer(tokenizer, bins=self.n_bin, min_action=self.min_action, max_action=self.max_action)
    ...
    def loss(self, pred_action_logits, labels, attention_mask=None):
        ...
        mask = torch.logical_and(labels > self.action_tokenizer.action_token_begin_idx, labels < self.action_tokenizer.action_token_end_idx)
        ...
        continuous_actions_pred = torch.tensor(
            self.action_tokenizer.decode_token_ids_to_actions(pred_action[mask].cpu().numpy())
        ).to(shift_logits.device)
        continuous_actions_gt = torch.tensor(
            self.action_tokenizer.decode_token_ids_to_actions(labels[mask].cpu().numpy())
        ).to(shift_logits.device)
        action_l1_loss = torch.nn.functional.l1_loss(continuous_actions_pred, continuous_actions_gt)
        
        return {"loss_arm": loss, "action_l1": action_l1_loss, "acc_arm": arm_acc, "acc_gripper": gripper_acc}
...
class FCDecoder(BasePolicyHead):
    ...
    def __init__(self, in_features, hidden_size, action_dim, down_sample, latent, fwd_pred_next_n, **kwargs):
    ...
    def forward(self, tok_seq, **kwargs):
        if len(tok_seq.shape) == 4:
            ...
            tok_seq = rearrange(tok_seq, 'b l n d-> (b l) n d') # reduce the seq_len dim
        elif tok_seq.dim() == 3:
        ...
        if seq_len is not None:
            ...
            gripper = rearrange(gripper, '(b l) (n d) -> b l n d', b=bs, l=seq_len, n=self.fwd_pred_next_n)
...
class LSTMDecoder(BasePolicyHead):
    ...
    def __init__(self, in_features, action_dim, down_sample, latent, fwd_pred_next_n, window_size, 
                 hidden_size=1024, num_layers=4, policy_rnn_dropout_p=0.0, **kwargs):
        ...
        self.rnn = lstm_decoder(in_features*latent, hidden_size*latent, num_layers, policy_rnn_dropout_p)
        self.actions = MLPTanhHead(self.hidden_size*latent, fwd_pred_next_n * (self.action_dim-1))
        self.gripper = MLPSigmoidHead(self.hidden_size*latent, fwd_pred_next_n)
    ...
    def forward(self, tok_seq, h_0=None, **kwargs):
        ...
        if self.down_sample == 'pooling':
            ...
            tok_seq = self.global_1d_pool(tok_seq.permute(0, 2, 1)) # bs*seq_len, n_tok, tok_dim -> bs*seq_len, tok_dim
...
class GPTDecoder(BasePolicyHead):
    ...
    def __init__(self, in_features, action_dim, down_sample, latent, fwd_pred_next_n, window_size, 
        ...
        from model.policy_head.trajectory_gpt2 import get_gpt_model
...
if __name__ == '__main__':
    net = FCDecoder(
        in_features=1024, hidden_size=1024, action_dim=7, down_sample='pooling', latent=1, fwd_pred_next_n=2, window_size=12,
    )
    import pdb; pdb.set_trace()
    net = LSTMDecoder(
